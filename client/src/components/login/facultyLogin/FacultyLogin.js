...
const FacultyLogin = () => {
  ...
  const onSubmit = async ({ username, password }) => {
    setLoading(true);
    await dispatch(facultySignIn({ username, password }, navigate));
    setLoading(false);
  };

  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 flex items-center justify-center p-4">
      {/* Home Button */}
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition transform hover:scale-105"
          >
